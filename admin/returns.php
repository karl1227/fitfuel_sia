                ...
                if ($totalItems > 0 && $refundedItems >= $totalItems) {
                    ...
                    try {
                        ...
                        if (getNotificationSetting('notif_order_status_enabled','1') === '1') {
                            $orderInfo = $pdo->prepare('SELECT user_id, custom_order_id FROM orders WHERE order_id = ?');
                            $orderInfo->execute([$orderId]);
                            $orderData = $orderInfo->fetch(PDO::FETCH_ASSOC);
                            if ($orderData) {
                                $title = 'Order ' . ($orderData['custom_order_id'] ?? ('#'.$orderId)) . ' Refunded';
                                $msg = 'Your refund has been processed successfully.';
                                $link = 'order_details.php?order_id=' . $orderId;
                                createNotification((int)$orderData['user_id'], 'order_status', $title, $msg, $link);
                            }
                        }
