    ...
    try {
        switch ($action) {
            case 'add_admin':
                ...
                audit_log('users', 'add_admin', 'success', [], ['username'=>$username, 'email'=>$email, 'role'=>$role, 'status'=>$status]);
                break;
            case 'edit_user':
                ...
                audit_log('users', 'edit_user', 'success', ['user_id'=>$userId], ['username'=>$username, 'email'=>$email, 'role'=>$role ?: $existing['role'], 'status'=>$status]);
                break;
            case 'delete_admin':
                if (!$isAdmin) { throw new Exception('Unauthorized'); }
                $userId = intval($_POST['user_id']);
                ...
                audit_log('users', 'delete_admin', 'success', ['user_id'=>$userId], []);
                break;
            default:
                // no-op
        }
