    ...
    if (empty($username_email) || empty($password)) {
        ...
        try {
            ...
            if ($user && password_verify($password, $user['password_hash'])) {
                ...
                // Audit log
                audit_log('auth', 'login_success', 'success', [], ['user_id'=>$user['user_id'], 'username'=>$user['username'], 'role'=>$user['role']]);
                ...
                audit_log('auth', 'login_failure', 'failure', [], ['username_email'=>$username_email]);
            }
        } catch (PDOException $e) {
            $error = "Login failed. Please try again.";
            audit_log('auth', 'login_error', 'failure', [], ['message'=>$e->getMessage()]);
        }
    }
}
?>
