...
CREATE TABLE IF NOT EXISTS user_privacy_settings (
  ...
  CONSTRAINT ups_user_fk FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE CASCADE
...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  $up=$pdo->prepare("INSERT INTO user_privacy_settings (user_id,profile_visibility,show_email,show_phone)
    VALUES (?,?,?,?)
  ...
  $up->execute([$user_id,$vis,$show_email,$show_phone]);
  $alert=['type'=>'success','msg'=>'Privacy settings saved.'];
}

$q=$pdo->prepare("SELECT * FROM user_privacy_settings WHERE user_id=?");
$q->execute([$user_id]);
$s=$q->fetch(PDO::FETCH_ASSOC) ?: ['profile_visibility'=>'public','show_email'=>1,'show_phone'=>0];
?>
<!DOCTYPE html>
<html lang="en"><head>
...
<body class="font-body bg-[#f6f6f6] text-slate-700">
  <!-- top bars -->
  <nav class="bg-white text-black py-2"><div class="container mx-auto px-4"><div class="flex justify-end space-x-6 text-sm"><a href="#" class="hover:text-emerald-400">Review</a><a href="#" class="hover:text-emerald-400">Help</a><a href="logout.php" class="hover:text-emerald-400">Logout</a></div></div></nav>
  <nav class="bg-black py-4"><div class="container mx-auto px-4"><div class="flex items-center justify-between"><a href="index.php"><img src="img/LOGO-Fitfuel.png" width="75"></a><div class="hidden md:flex items-center space-x-8"><a href="index.php" class="text-white hover:text-emerald-600">Home</a><a href="shop.php" class="text-white hover:text-emerald-600">Shop</a><a href="#" class="text-white hover:text-emerald-600">About</a><a href="#" class="text-white hover:text-emerald-600">Contact</a></div><div class="flex items-center space-x-4"><a href="cart.php" class="p-2 text-white hover:text-emerald-600"><i class="fas fa-shopping-cart text-xl"></i></a><a href="profile.php" class="p-2 text-white hover:text-emerald-600"><i class="fas fa-user text-xl"></i></a></div></div></div></nav>
  ...
  <div class="container mx-auto px-4 py-8 grid grid-cols-1 md:grid-cols-4 gap-6">
    <aside class="md:col-span-1 bg-white rounded-lg border border-gray-200 p-4">
      ...
      <nav class="text-[15px]">
        ...
        <a href="privacy_settings.php" class="sidebar-link <?php echo active_link('privacy_settings.php');?>">Privacy Settings</a>
    ...
    <section class="md:col-span-3 bg-white rounded-lg border border-gray-200">
      ...
      <?php if($alert['type']==='success'):?><div class="mx-6 mt-4 rounded bg-emerald-50 text-emerald-700 px-4 py-3 border border-emerald-200"><?php echo h($alert['msg']);?></div><?php endif;?>
      ...
      <form class="p-6 max-w-xl" method="post">
        <label class="block mb-4">
          ...
          <select name="profile_visibility" class="w-full border rounded px-3 py-2">
            <option value="public"  <?php if($s['profile_visibility']==='public') echo 'selected';?>>Public</option>
            <option value="friends" <?php if($s['profile_visibility']==='friends') echo 'selected';?>>Friends only</option>
            <option value="private" <?php if($s['profile_visibility']==='private') echo 'selected';?>>Private</option>
          </select>
        </label>
        <label class="flex items-center gap-2 mb-3">
          <input type="checkbox" name="show_email" <?php if($s['show_email']) echo 'checked';?>>
          <span>Show my email on profile</span>
        </label>
        <label class="flex items-center gap-2 mb-6">
          <input type="checkbox" name="show_phone" <?php if($s['show_phone']) echo 'checked';?>>
