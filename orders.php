...
try {
	if ($action === 'update_status') {
		...
		audit_log('orders', 'update_status', 'success', ['order_id'=>$orderId], ['status'=>$newStatus, 'payment_status'=>$paymentStatus, 'payment_method'=>$paymentMethod, 'estimated_delivery_date'=>$deliveryDate]);
	...
	if ($action === 'update_payment') {
		...
		audit_log('orders', 'update_payment', 'success', ['order_id'=>$orderId], ['payment_status'=>$paymentStatus, 'payment_method'=>$paymentMethod]);
	...
	if ($action === 'update_shipping') {
		...
		audit_log('orders', 'update_shipping', 'success', ['order_id'=>$orderId], ['shipping_address'=>$address, 'estimated_delivery_date'=>$eta]);
	...
	if ($action === 'approve_return') {
		...
		audit_log('orders', 'approve_return', 'success', ['order_id'=>$orderId], ['return_reason'=>$reason, 'refund_amount'=>$refund]);
	...
	if ($action === 'reject_return') {
		...
		audit_log('orders', 'reject_return', 'success', ['order_id'=>$orderId], []);
	...
	if ($action === 'cancel_order') {
		...
		audit_log('orders', 'cancel_order', 'success', ['order_id'=>$orderId], ['status'=>'cancelled']);
	...
	if ($action === 'create') {
		...
		try {
			...
			$pdo->commit();
			audit_log('orders', 'create', 'success', ['user_id'=>$userId], ['order_id'=>$orderId ?? null, 'items_count'=>count($items), 'total_amount'=>$total]);
			$message = 'Order created.';
		} catch (Throwable $e) {
			$pdo->rollBack();
			audit_log('orders', 'create', 'failure', ['user_id'=>$userId], ['error'=>$e->getMessage()]);
			throw $e;
		}
	}
} catch (Throwable $ex) {
	$error = $ex->getMessage();
	audit_log('orders', 'error', 'failure', [], ['message'=>$error]);
}
